...
//MQTT
void DS1::Publish(byte *sendPayload, int len, byte topicId)
{
	this->sendBuffer[0] = (byte)len + MQTT_HEADER_SIZE;
	this->sendBuffer[1] = (byte)PublishType;
	this->sendBuffer[2] = topicId; //topic id
	for (int i = 0; i<len; i++)
	{
		this->sendBuffer[i + MQTT_HEADER_SIZE] = sendPayload[i];
	}
	this->SendDataToMaster(this->sendBuffer, len + MQTT_HEADER_SIZE);
}
